...
const renderSearchResult = (item) => `
  <article class="bl-search-result bl-panel bl-panel-ghost">
...
const wireSearch = (state) => {
  document.querySelectorAll('[data-bl-search-form]').forEach((form) => {
    form.addEventListener('submit', async (event) => {
      ...
      target.innerHTML = `
        <div class="bl-empty-state bl-panel bl-panel-ghost">
          <h3>Searching</h3>
          <p>Looking through ${escapeHTML(state.site?.title || 'the publication')} for “${escapeHTML(query)}”.</p>
        </div>
      `;
...
const buildTOC = () => {
  const prose = document.querySelector('[data-bl-article-prose]');
  const toc = document.querySelector('[data-bl-toc]');
  const panel = document.querySelector('[data-bl-toc-panel]');
  ...
  toc.innerHTML = headings
    .map(
      (heading) => `
        <a class="bl-toc-link bl-toc-link-${heading.tagName.toLowerCase()}" href="#${escapeHTML(heading.id)}">
...
const bootstrap = async () => {
  ...
  document.documentElement.dataset.foundryTheme = 'bloomlight';
  wireSearch(state);
  buildTOC();
